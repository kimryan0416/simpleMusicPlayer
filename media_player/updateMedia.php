<?php
	...
	if ( array_key_exists($song_ext, $audio_types) ) {
		// file is audio type
		$url = PATH_TO_MEDIA_FOR_FILES."Music/".$new_basename.".".$song_ext;
		$type = "audio";
	} else if ( array_key_exists($song_ext, $video_types) ) {
		$url = PATH_TO_MEDIA_FOR_FILES."Movies/".$new_basename.".".$song_ext;
		$type = "movie";
	}

	$arraytoAssociate = sizeof($albums[$type]);
	$arrayToPoint = 0;
	$albumInArray = false;
	foreach((array)$albums[$type] as $key=>$albumToSee) {
		if ($albumToSee["name"] == $album) {
	...
	if ( !$albumInArray ) {
		...
		$albumID++;
		$arrayToPoint = sizeof($albums[$type]) - 1;
	}

	/*
	$imageForSong = PATH_TO_ASSETS_FOR_FILES."default_album_art.jpg";
	...
	*/
	...
	$albums[$type][$arrayToPoint]["list"][] = $songID;

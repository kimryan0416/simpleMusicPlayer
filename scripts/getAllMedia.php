			...
			if ($row['medium'] == 0) {
				if ($row['dynamic_lyrics_toggle'] == 1) {
					...
					foreach($lyrics_array as $lyrics_segment) {
						...
						if ( preg_match("/\[([0-5][0-9]):([0-5][0-9])(.([0-9]{0,3}))?\]({.*?})?/", $lyrics_segment_array[0], $lyrics_segment_time) ) {
							if ( !isset($lyrics_segment_time[5]) || $lyrics_segment_time[5] == '' ) $lyrics_segment_style = 'black'; 
							else {
								$lyrics_segment_style = str_replace(array('{', '}'), array('', ''), $lyrics_segment_time[5]);
								if ($lyrics_segment_style == 'yellow') $lyrics_segment_style = 'rgb(254,223,0)';
								else if ($lyrics_segment_style == 'pink') $lyrics_segment_style = 'rgb(255,0,144)';
							}
						...
						else $lyrics_starting_time = '59:59.999';
						$lyrics_starting_time = convertToMilliseconds($lyrics_starting_time);
						$lyrics_starting_times[] = $lyrics_starting_time;
						if ( preg_match("/\[NOTEXT\]/", $lyrics_segment_array[0], $lyrics_segment_noText) ) $noText = 'noText';
						...
						else $lyrics_HTML = '<span class="lyric_segment lyric_segment_'.$lyrics_starting_time.' noText" style="color:'.$lyrics_segment_style.';">';
						$lyrics_HTML .= '</span>';
						$lyrics .= $lyrics_HTML;
					}
					...
					foreach ($lyrics_array as $lyric_segment) {	$lyrics .= '<span class="lyric_segment lyric_segment_-1 noText">' . $lyric_segment . '</span>';	}
					$lyrics .= '<span class="lyric_segment lyric_segment_-1 noText"></span>';
					$row['lyrics'] = $lyrics;
				...
				if ($row['dynamic_lyrics_toggle'] == 1) {
					...
					foreach($lyrics_array as $lyrics_segment) {
						...
						$noText = '';
						if ( preg_match("/\[([0-5][0-9]):([0-5][0-9])(.([0-9]{0,3}))?\]({.*?})?/", $lyrics_segment_array[0], $lyrics_segment_time) ) {
							if ( !isset($lyrics_segment_time[5]) || $lyrics_segment_time[5] == '' ) $lyrics_segment_style = 'white'; 
							else {
								$lyrics_segment_style = str_replace(array('{', '}'), array('', ''), $lyrics_segment_time[5]);
								if ($lyrics_segment_style == 'yellow') $lyrics_segment_style = 'rgb(254,223,0)';
								else if ($lyrics_segment_style == 'pink') $lyrics_segment_style = 'rgb(255,0,144)';

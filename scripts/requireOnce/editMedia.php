<?php
	...
	if ( !isset($get) || ( $get!=5 && $get!='editMedia' ) ) {
	...
	$icon = filter_input(INPUT_GET,'icon',FILTER_VALIDATE_INT);
	$song = filter_input(INPUT_GET,'song',FILTER_VALIDATE_INT);

	// Check if we're editing the icon or editing media file information
	if(isset($icon)) {

		// Was the proper $icon gotten (aka 1)? If not, we expect malicious intent and return early with error
		if ( $icon != 1 ) {
			print(returnError($db,'mediaEdit - icon GOTTEN but not expected value'));
			return;
		}

		// if we don't have to change the current icon (because we either canceled the edit or we didn't upload a new one)
		$id = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);
		$change = filter_input(INPUT_GET, 'change', FILTER_VALIDATE_INT);

		if ( !isset($id) ) {
			print(returnError($db,'mediaEdit - Proper ID not received'));
			return;
		}
		else if (!isset($change)) {
			print(returnError($db,'mediaEdit - Proper CHANGE not received'));
			return;
		}

		// need to detect if we really have to change the icon or not via $change
		if ($change != 1) {
			print(returnSuccess($db,'Icon Request Recieved, ID found, but we will not change the icon'));
			return;
		}
		...
		if(getimagesize($file['tmp_name']) === false) {
			print(returnError($db,'mediaEdit - This file is not an image; please select an image file'));
			return;	
		}
		...
		if ( $song != 1 ) {
			print(returnError($db,'mediaEdit - song GOTTEN but not expected value'));
			return;
		}
		...
		if (!isset($reload)) {
			print(returnError($db,'mediaEdit - reload GOTTEN but not expected value'));
			return;
		}
		else if (!isset($newArtUploaded)) {
			print(returnError($db,'medaiEdit - newArtUploaded GOTTEN but not expected value'));
			return;
		}
		...
		print(returnError($db,'mediaEdit - neither ICON nor SONG were GOTTEN'));
		return;
	}
